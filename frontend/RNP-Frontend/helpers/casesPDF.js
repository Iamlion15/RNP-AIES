...
const CompleteCasesPDF = (dataa, pData) => {
  const unit = 'mm';
  const size = 'a4';
...
const newHeight = newWidth / aspectRatio;
  ...
  // Page Title
  const titleText = "COMPLETED CASES REPORT ";
  doc.setFontSize(15);
  ...
  const printData = dataa.map((data, index) => {
    ...
    const completedon=formatDateToCustomFormat(data.completedCaseOn)

    return [index + 1, names, location, tookplaceon,completedon];
  });

  // Auto-generate a table in the PDF using autoTable
  doc.autoTable({
    head: [['No.','OGP NAMES','LOCATION',  'TOOK PLACE ON','COMPLETED ON']],
    body: printData,
    startY: startY,
  ...
  // Save PDF
  doc.save(`Cases-reported-${currentDate}.pdf`);
};

export default CompleteCasesPDF;
